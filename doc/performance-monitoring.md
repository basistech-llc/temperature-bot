...
An existing lock created by an older release may be mode `0600` and owned by a
different service account. Before restarting upgraded processes, keep old and
new code interoperable with `sudo chmod 0666 /tmp/temperature-bot-ae200.lock`.
After every process that accesses the AE-200 runs the read-only-lock code, set
the final mode with `sudo chmod 0444 /tmp/temperature-bot-ae200.lock`.
