...
- `AE200_COMMAND_LOCK_PATH`: host-wide advisory lock shared by every process
  and service account that can reach the controller. The lock file contains no
  data and is deliberately read-only for all users; `flock()` locks its inode
  without writing it. Production defaults to
  `/run/lock/temperature-bot/ae200.lock`, provisioned by
  `etc/tmpfiles.d/temperature-bot.conf`.
...
The AE-200 controller accepts WebSocket request/response exchanges from several
local processes: Gunicorn workers, scheduled runners, and developer services.
`ae200_command_lock()` takes an exclusive advisory `flock()` around each
complete exchange so two processes on the same host cannot overlap controller
commands or consume one another's responses.

The lock file is an empty inode. It contains no application state and is never
written. It does not lock SQLite, survive as an acquired lock after a process
exits, or coordinate processes on different hosts. The kernel releases the
advisory lock when the file descriptor closes, including after a process
crash. Every cooperating AE-200 caller must open and lock the same path for the
serialization guarantee to hold.

Production uses `/run/lock/temperature-bot/ae200.lock`. The tmpfiles policy
creates its parent directory as `root:root 0755` and the empty file as
`root:root 0444` at boot. Read-only mode is sufficient because `flock()` acts
on the open inode rather than file contents. Root ownership and the
non-writable directory prevent service accounts from replacing the shared
inode. If the policy was not installed or applied, AE-200 operations fail
closed instead of silently using a private per-process lock.

Install the tmpfiles policy and create the runtime lock before restarting an
upgraded process:
...
The obsolete `/tmp/temperature-bot-ae200.lock` is not used after every AE-200
caller has upgraded and may then be removed.

Developer services on the shared server use the same root-provisioned lock, so
the `simsong`, `deg`, and `temperature_bot` processes continue to serialize
their AE-200 requests. On developer machines, the supported `make local-dev`,
`make local-live-dev`, and `make live-dev-runner` targets instead create and
select `.tmp/runtime/ae200.lock` inside the checkout; they do not require root
or a Linux `/run` directory. Filesystem locks coordinate only processes on the
same host.
