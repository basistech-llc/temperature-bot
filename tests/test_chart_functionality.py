...
"""
import json
from conftest import client_with_db  # noqa: F401 # pylint: disable=unused-import

def test_status_api_endpoint_for_devices(client_with_db):  # noqa: F811 # pylint: disable=unused-argument
...
def test_chart_page_with_device_dropdown(client_with_db):  # noqa: F811 # pylint: disable=unused-argument
    """Test that the chart page loads with device dropdown"""
    response = client_with_db.get('/chart')
    assert response.status_code == 200
...
def test_chart_page_with_specific_devices(client_with_db): # noqa: F811 # pylint: disable=unused-argument
    """Test that the chart page loads with specific devices"""
    response = client_with_db.get('/chart?device_ids=1,2')
    assert response.status_code == 200
...
def test_temperature_api_with_multiple_devices(client_with_db): # noqa: F811 # pylint: disable=unused-argument
    """Test that the temperature API works with multiple device IDs"""
    response = client_with_db.get('/api/v1/temperature?device_ids=1,2')
    assert response.status_code == 200
